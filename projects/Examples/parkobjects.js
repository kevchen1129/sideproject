...
export class GrCarousel extends GrObject {
    ...
    constructor(params={}) {
		...
		let base_mat = new T.MeshStandardMaterial({color:"pink", metalness:0.3, roughness:0.8});
		let base = new T.Mesh(base_geom, base_mat);
		base.translateY(0.5);
		carousel.add(base);
		...
		let platform_mat = new T.MeshStandardMaterial({color:"white", metalness:0.3, roughness:0.8});
		let platform = new T.Mesh(platform_geom, platform_mat);
		platform_group.add(platform);
		...
		let opole_mat = new T.MeshStandardMaterial({color:"pink", metalness:0.8, roughness:0.5});
		let opole;
		let num_poles = 10;
		let poles = [];
		let horses=[];
		let horseGeom=new T.BoxGeometry(0.8,0.8,0.8);
		let horseMat=new T.MeshStandardMaterial({color:"lightblue"});
		let horse;
		...
		for (let i = 0; i < num_poles; i++)
		{
			...
			platform_group.add(opole,horse);
		...
		this.advance=function(delta, timeOfDay) { 
			let i=0;
			let a=performance.now()/1200;
			this.whole_ob.rotateY(0.0008*delta); 
			this.horses.forEach(function(x){
				 i+=2;
				x.position.y =0.7*Math.sin(a+i)+1.2;
			})
			i=0;
